import React from 'react';
import {
  Text,
  TouchableOpacity,
  View,
  Image,
} from 'react-native';
import BaseComponent from '../../../common/BaseComponent';
import IMAGE_CONSTANT from '../../../../constants/images';
import styles from '../../style';
import PROGRESS from '../../../../constants/progress';
import { getDateString } from '../../../../helpers/shipment.helper';
import Attactments from '../../../booking/attactments/Attactments';
import MyComment from '../MyComment';
import { DATE_TIME_FORMAT_VN, DATE_TIME_FORMAT } from '../../../../constants/app';

class Booked extends BaseComponent {
  constructor(props) {
    super(props);
    this.state = {
      expanded: props.collapseSection === PROGRESS.SECTION.BOOKED || false,
    };
  }

  componentDidUpdate(prevProps) {
    const { collapseSection } = this.props;
    if (collapseSection !== prevProps.collapseSection) {
      this.setState({
        expanded: collapseSection === PROGRESS.SECTION.BOOKED || false,
      });
    }
  }

  getTextLanguage = (key) => {
    const { languageCode } = this.props;
    return this.getLanguageLocation(key, languageCode);
  }

  renderExpandedDefault = () => {
    const {
      source,
      actions,
      languageCode,
      countryCode,
      shipmentID,
    } = this.props;
    return (
      <>
        <View style={[styles.mt30, styles.mb10]}>
          <View style={[styles.flex, styles.alignItemsCenter]}>
            <Text style={[styles.formHeader, { top: 2 }]}>
              {this.getTextLanguage('progress.driver_comments')}
            </Text>
          </View>
          <View style={[styles.whiteBg, styles.paddingHorizontal20,
            styles.paddingVertical30, styles.mb30, styles.grayBorder]}
          >
            <View style={styles.formGroupInput}>
              <Text style={[styles.defaultSize, styles.defaultTextColor, styles.bold, styles.mb10]}>
                {this.getTextLanguage('progress.notes')}
              </Text>
              <Text style={[styles.smallSize, styles.defaultTextColor]}>
                {source.driverNotes || ''}
              </Text>
            </View>
            <View style={[styles.formGroupInput, styles.mt20]}>
              {source.driverAttachments.length > 0 ? (
                <Attactments
                  readOnly
                  progress
                  languageCode={languageCode}
                  photos={source.driverAttachments}
                />
              ) : null}
            </View>
          </View>
        </View>
        <MyComment
          actions={actions}
          section={PROGRESS.SECTION.BOOKED}
          languageCode={languageCode}
          countryCode={countryCode}
          source={source}
          shipmentID={shipmentID}
        />
      </>
    );
  }

  renderExpanded = () => (
    <>
      <View style={[styles.mt25, styles.line]} />
      {this.renderExpandedDefault()}
    </>
  )

  toggleCollapse = () => {
    const { actions, shipmentStatus, collapse } = this.props;
    this.setState((prevState) => ({
      expanded: !prevState.expanded
    }), () => {
      const { expanded } = this.state;
      collapse(PROGRESS.SECTION.BOOKED, expanded);
    });
  }

  render() {
    const {
      source,
      countryCode,
      languageCode,
    } = this.props;
    const { expanded } = this.state;
    return (
      <View
        style={[
          styles.mb20,
          styles.paddingHorizontal20,
          styles.paddingVertical20,
          styles.lightGreenBg,
        ]}
      >
        <TouchableOpacity
          activeOpacity={1}
          onPress={this.toggleCollapse}
        >
          <View style={[styles.flex, styles.alignItemsCenter]}>
            <View
              style={[
                styles.flexOne,
                styles.flex,
                styles.alignItemsCenter,
                styles.mr5,
              ]}
            >
              <View style={styles.mr10}>
                <Image source={IMAGE_CONSTANT.shipmentBook} />
              </View>
              <View>
                <Text
                  style={[
                    styles.defaultSize,
                    styles.mainColorText,
                    styles.bold,
                  ]}
                >
                  {this.getTextLanguage('progress.booked')}
                </Text>
                <Text style={[styles.smallSize, styles.defaultTextColor]}>
                  {getDateString(source.bookedDate, countryCode, languageCode, languageCode === 'vi' ? DATE_TIME_FORMAT_VN : DATE_TIME_FORMAT)}
                </Text>
              </View>
            </View>
            {expanded ? (
              <Image
                source={IMAGE_CONSTANT.arrowUp}
                style={{ width: 24, height: 24 }}
                resizeMode="contain"
              />
            ) : (
              <Image
                source={IMAGE_CONSTANT.arrowDown}
                style={{ width: 24, height: 24 }}
                resizeMode="contain"
              />
            )}
          </View>
        </TouchableOpacity>
        {expanded && this.renderExpanded()}
      </View>
    );
  }
}

export default Booked;
