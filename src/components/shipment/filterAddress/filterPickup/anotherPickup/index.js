...
class AnotherPickup extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleRemovePickup = (index) => () => {
    this.removePickupAddress('another', index);
    if (this.googleRef) {
  ...
  removePickupAddress = (type, index) => {
    const { removeAddress, anotherPickup } = this.props;
    this.setState({
      listViewDisplayed: false,
    });
    if (index === 0) {
      if (anotherPickup[index + 1] && anotherPickup[index + 1]) {
        this.setState({
          valueRange: anotherPickup[index + 1].radius,
        });
      }
    }
    this.setState({
  ...
  render() {
    const {
      languageCode, item, index, countryCode, scrollRef
    } = this.props;
    const {
      ...
      <View style={[styles.mt10, styles.flex, styles.alignItemsCenter]} key={`address-${index + 1}`}>
        <View style={expandedRadius ? { flex: 3 } : styles.flexOne}>
          <GooglePlacesAutocomplete
            ...
            minLength={2}
            autoFocus={false}
            onPress={(data) => this.onSelectAddress(data, { isPickup: true, type: 'another', index })}
            query={{
              key: SYSTEM.GOOGLE_API_KEY,
                ...
                return (
                  ...
                  >
                    <TouchableOpacity
                      ...
                      onPress={this.handleRemovePickup(index)}
                    >
            ...
            textInputProps={{
              ...
              onFocus: () => {
                ...
                // });
                // this.textInput.measureInLayout(findNodeHandle(this.textInput, (x, y, w, h) => {
                //   console.log('x, h, w, h', x, y, w, h);
                // }));
              },
              onBlur: () => {
        ...
        {item.address && showRadius && (
          ...
          >
            {expandedRadius
              ? this.singleRangeSlider(true, 'another', index)
              : (
                <TouchableOpacity
                  activeOpacity={1}
